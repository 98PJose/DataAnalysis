# PRACTICA 3. ANALISIS DE COMPONENTES PRINCIPALES Y ANALISIS FACTORIAL

# EJEMPLO 1: RATIOS ("FracasoEmpresarial.txt")

# Objetivo: El objetivo de esta segunda practica es aprender a aplicar e interpretar
...
# Para cada empresa se facilita información de los siguientes ratios:
    # "bai.at","bai.fp","ac.at","t.at","v.fp","ac.pc","t.pc","fm.v","pe.pt","cf.pt","fm.at","v.at","v.ac" y "lnat"
...
# Comenzamos con los primeros pasos de siempre:
# Cargar las librerias necesarias (en este caso, psych, corrplot y semPlot)
# Cambiar el directorio de trabajo 
# Leer datos (archivo "ratios.txt" y asignarlo al data frame ratios) 
...
ratios <- read.table("FracasoEmpresarial.txt",h=T)#leemos los datos y le asignamos el nombre ratios
...
which.max(cor)
corrplot.mixed(cor, upper="ellipse") #Pueden parecer pequeñas las correlaciones, pero hay que tener en cuenta que son datos reales
...
n <- dim(ratios)[1] # Necesitamos el numero de observaciones para el test de Bartlett

# Prueba de esfericidad de Bartlett
cortest.bartlett(cor(ratios),n)	#Hipotesis nula es que las correlaciones son 0, por tanto, interesa rechazarla.
...
View(ratios.cp)
summary(ratios.cp) 
get_eigenvalue(ratios.cp)	# Esta funcion del paquete factoextra nos muestra los autovalores (varianza de cada componente, % sobre varianza total y varianza acumulada.
#Analisis grafico
...
fviz_eig(ratios.cp, choice="eigenvalue",ncp=14, addlabels = T) #Otra funcion alternativa para el grafico con el paquete factoextra
fviz_eig(ratios.cp, choice="variance", ncp=14, addlabels = T) #Screeplot pero representando % de varianza #El anterior muestra el autovalor
fviz_pca_biplot(ratios.cp) #Biplot de individuos y variables con las dos primeras dimensiones #Representa los dos componentes principales
...
ratios.cp$loadings[,1:2] #Si seleccionamos los dos primeras componentes. Vemos sus correlaciones con las variables
