...
author: "Pedro José Piqueras Martínez"
date: "25/2/2023"
output: 
  html_document:
    toc: true
...
En el siguiente código cargamos los datos y los mostramos. La función `read_excel`
carga los datos,`datatable` nos los muestra con un formato más visual y `str` nos 
dice el tipo de cada variable.
...
Algunas variables aparecen como `chr` cuando en realidad son factores, por tanto,
las convertimos a factor usando `mutate_if` y `as.factor`.
...
Usando nuestra función `NOMBRE_VARIABLES` renombramos nuestras variables para 
facilitar la reproducibilidad del trabajo y evitar problemas ortográficos.
Creamos una tabla llamada `nombres_df` para poder identificar las variables. 
...
Creamos una clave primaria que nos servirá posteriormente para recuperar los 
outliers
...
datatable(covarianzas, fillContainer=TRUE)
...
(rango intercuantílico) son potenciales outliers.
...
Vamos a realizar un análisis de componentes principales (PCA) para tratar de 
representar nuestros datos en 2 dimensiones mediante un biplot:
...
El porcentaje de variabilidad explicada por los dos primeros componentes es muy
reducida, aun así vamos a realizar el biplot con ánimo de practicar:
...
Aquellos valores que toman valores muy elevados o combinaciones extrañas son 
potenciales outliers.

## 2.2 Métodos estadísticos.

En este apartado vamos a emplear métodos que trabajan mediante contrastes de hipótesis.

### 2.2.1 Método univariante de la distancia intercuartílica.

Este método para la detección de outliers univariantes se basa en ver que valores
se alejan más de un múltiplo de veces sobre la distancia intercuartílica.

Mediante nuestra función `OUTLIERS_IQR` buscamos aquellas observaciones que se
alejan por encima o por debajo más de 1.5 veces la distancia intercuartílica.
...
Hemos programado la función `OUTLIERS_IQR_TOTAL` que aplica a todas las columnas
mediante un loop. La salida es una lista con tantos elementos como variables y 
que guardan los potenciales outliers univariantes para cada variable.
...
No aplicamos esta técnica para retirar outliers pues un potencial outlier univariante
no tiene por qué serlo en una dimensión múltiple ya que puede pertenercer a una
tendencia o correlación.
...
Usando nuestra función `dist_mediana` podemos calcular la observación con mayor
distancia absoluta a la mediana por variable:

```{r}
# observacion con mayor distancia a la mediana por variable

df %>% select_if(is.numeric) %>% dist_mediana() %>% datatable()
```
...
### 2.2.2. Contrastes de normalidad.
...
### 2.2.3 Contrastes para outliers univariantes.

En este apartado vamos a trabajar con el test de Grubbs cuya Ho es existe un 
único outlier univariante. Si queremos detectar más de uno, presenta el problema
del enmascaramiento, los outliers pueden alterar la media y hacer fallar el contraste.

El test de Grubbs es muy intuitivo pues se basa en el valor tipificado que más
se aleja de la media.

Nuestra función `GRUBBS_TABLA` crea una tabla resumen con los resultados para
cada variable.
...
Si quisieramos eliminar outliers con este método podríamos usar nuestra función
`GRUBBS_REMOVER` que emplea `GRUBBS_TABLA` para ver que variables rechazan el test
y entonces elimina la observación más valor el estadístico de Grubbs.

```{r}
# eliminar outliers segun test Grubbs una vez
# df = GRUBBS_REMOVER(df,0.05) 
```

No lo haremos pues los métodos univariantes obvian las correlaciones y tendencias.

### 2.2.4 Distancia de Mahalanobis.

La distancia de Mahalanobis es un tipo de distancia que tiene en cuenta las 
correlaciones entre variables, de forma que si un valor está lejos del centroide
por ser un valor extremo en una correlación, no tiene una distancia de Mahalanobis
demasiado elevada. De forma visual serían elipses entorno al centroide, al contrario
que la distancia euclídea donde son círculos.

La distancia de Mahalanobis asume la existencia de una distribución normal
multivariante, por ello tenemos que contrastar si es cierto.

```{r}
mvn(df %>% select(where(is.numeric)), mvnTest = "hz")$multivariateNormality
```

La distancia de Mahalanobis se estima mediante la función `mahalanobis`
...
La distancia de Mahalanobis estándar se distribuye según una chi-cuadrado de p
grados de libertad, siendo p el número de variables cuando los datos se distribuyen
de forma normal. Dado que vamos a contrastar simultáneamente varias observaciones 
provocamos un FWER, por tanto, debemos corregir el nivel de significación.

`qchisq` calcula el valor que deja probabilidad a la izquierda, por tanto recibe
como argumento 1-alfa.
...
Podemos analizar gráficamente las observaciones según su valor de distancia.
...
Podemos ver quines superan la distancia límite y cuántos son:
...
Si quisieramos quitar outliers con esta técnica usariamos el siguiente código.
Recordemos que empleamos una correción del nivel de significación para n outliers.
Cómo mucho podemos quitar con criterio estadístico esos n.

No lo vamos a ejecutar pues emplearemos la distancia robusta.
...
### 2.2.5 Distancia de Mahalanobis Robusta.

La distancia de Mahalanobis robusta funciona mejor ante la presencia de outliers
y la ausencia de normalidad.
...
La distancia de Mahalanobis robusta la estimamos utilizando el método del 
determinante mínimo de covarianza (MCD) 
...
Típicamente la distribución de este estadístico es desconocida pero se puede 
aproximar mediante simulaciones o bootstrap. Por motivos de simplicidad, vamos
a trabajar (erróneamente) con una chi-cuadrado de p grados de libertad como sucede 
con la distancia no robusta en caso de normalidad.

Dado que hacemos múltiples contrastes debemos corregir el nivel de significación
para reducir el FWER.
...
Aunque los datos no sean normales, las observaciones con una distancia muy 
extrema son potencialmente outliers, el problema vendría en caso de distancia
moderada.
...
Podemos estudiar quiénes rebasan el límite y cuántos son:
...
Para retirar outliers con este método empleamos el siguiente código.
Recordemos que empleamos una correción del nivel de significación para n outliers.
Cómo mucho podemos quitar con criterio estadístico esos n.
...
El siguiente gráfico muestra la puntuación LOF de las observaciones ordenadas
de forma ascendente, no es muy útil cuando hay muchas observaciones.
...
## 2.4 Métodos basados en clúster.

Vamos a trabajar con dos métodos, la distancia al centroide y la distancia al
medoide.

En primer lugar, necestimos un análisis clúster previo, nosotros lo vamos a hacer
a modo de ejemplo con K-Medias, pero podrían utilizarse otros métodos. En consecuencia,
las funciones que hemos programado se basan en la salida de este método y podrían
no funcionar con otras librerías.

Mediante el estadístico de Hartigan seleccionamos el número de clústers:
...
El número de clúster para el que se reduce significativamente la pendiente es
el deseado.

Hacemos el análisis clúster y observamos sus centroides:
...
datatable(data.frame(km_clusters$centers),fillContainer=TRUE) # centroides
```

### 2.4.1 Distancia al centroide

Mediante nuestra función `OUTLIERS_KMEDIAS` podemos buscar las n observaciones
con mayor distancia al centroide.
...
Si quisieramos eliminarlos usaríamos el siguiente código:
...
Nuestra función `OUTLIERS_KMEDOIDES` detecta las n observaciones más alejadas a
sus medoides, para obtener los medoides utiliza la función `pam`.

```{r}
OUTLIERS_KMEDOIDES(df,n,k)
```

Si quisieramos eliminar outliers lo haríamos de la siguiente manera.
...
## 2.5 Análisis de las anomalias

En primer lugar creamos un data frame con los outliers mediante `anti_join` y nuestra columna `ID`
...
# los mostramos
datatable(df_outliers,fillContainer=TRUE)

```

Además, retiramos la columna ID y tipificamos de nuevo las variables. Cómo llegados
a este punto ese código es repetitivo, queda oculto.
...
Lo comparamos con los datos principales:
...
consecuencia, el objetivo de los siguientes apartados es solucionar este problema.

Definimos un objeto con nuestra clase minoritaria para ir usandolo:
...
## 3.1 Reducción de observaciones

En este apartado mostramos métodos que equilibran las probabilidades mediante
reducir las observaciones de clase mayoritaria.

### 3.1.1 Vínculos de Tomek

El algoritmo de los vínculos de Tomek consiste en buscar la observación de clase
mayoritaria más cercana a cada una de las observaciones de clase minoritaria y
retirarlas, limpiando así el espacio en que se encuentran las minoritarias.

Este algoritmo lo hemos implementado en nuestra función `tomek`:
...
No lo empleamos pues preferimos no retirar observaciones, pero se haría con el 
siguiente código:
...
### 3.1.2 Eliminación aleatoria.

Una alternativa consiste en eliminar de forma aleatoria observaciones mayoritarias,
esto podemos hacerlo con `sample`.

Nuestra función `n_deseada` calcula el número de observaciones a retirar o crear
para lograr las probabilidades a priori deseadas:
...
Recogemos n observaciones mayoritarias aleatorias de la siguiente manera:
...
Para retirarlas se hace de esta manera:
...
## 3.2 Generación de observaciones.

Los métodos basados en generar observaciones, como su nombre indica, se basan en
crear observaciones sintéticas.
...
### 3.2.1 Datos normales.

Si nuestras variables siguen una distribución normal puede ser una buena idea 
generar datos normales aleatorios.

Nuestra función `GENERADOR_RNORM` calcula la media y desviación poblacional 
para cada variable y con un loop emplea `rnorm` para generar `n` datos normales
aleatorios para cada variable.
...
Solo sirve con variables numéricas, podríamos unir los datos de la siguiente 
forma:
...
Una forma de generar datos es tomar muestras aleatorias con remplazamiento de 
nuestra clase minoritaria.
...
Para unir los datos hacemos lo siguiente:
...
El algoritmo SMOTE consiste en buscar los K vecinos más cercanos a una observación
minoritaria y generar un dato sintético mediante una combinación lineal aleatoria
de dichos vecinos.

Para el valor de K tomamos la heurística de la raíz del número de observaciones.
...
Solo sirve para variables numéricas.

Si quisieramos usar este método unimos los valores de la siguiente manera:
...
Tras generar las probabilidades a priori tenemos que hacer algunos pasos previos
a la validación cruzada. Que son tipificar de nuevo y volver a mezclar los datos.
Como ya hemos usado este código, queda oculto.
...
Hemos creado un índice con el que separar los datos:
...
Si dispusieramos de un número reducido de variables podríamos hacer un gráfico
...
Para predecir en el conjunto de entrenamiento y test hacemos lo siguiente:
...
Con nuestra función `PRECISION` podemos medir el error:
...
En adelante trabajaremos con `confusionMatrix` del paquete `caret` por ser 
más eficiente y completa.

A continuación guardamos las medidas de precisión:
...
Esto nos permite crear una tabla para almacenar las medidas de precisión:
...
Si tuvieramos pocas variables podríamos representar gráficamente:
...
Si tuvieramos pocas variables podríamos representar:
...
Lo estimamos con `rpart`, en primer lugar un árbol completo:
...
Si tiene muchos nodos puede ser un poco contraintuitivo.
...
Requerimos unos parámetros previos, en adelante no se repetirán:
...
Redes muy sencillas podrían representarse:
...
Hacemos algunas operaciones y creamos algunos objetos necesarios:
...
Antes de ejecutar el loop creamos algunos objetos necesarios para su ejecución
...
Después, creamos una tabla con los errores promedios:
...
La podemos mostrar ordenada según el error:
